...
// 색상 계산 유틸 (파랑→빨강 그라데이션)
function getHeatColor(value: number, min: number, max: number) {
  if (max === min) return "#3b82f6"; // 모든 값 동일하면 기본 파랑

  const ratio = (value - min) / (max - min); // 0~1

  const r = Math.round(59 + ratio * (239 - 59)); // 59 → 239
  const g = Math.round(130 + ratio * (68 - 130)); // 130 → 68
  const b = Math.round(246 + ratio * (68 - 246)); // 246 → 68
