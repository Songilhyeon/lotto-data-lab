...
const features = [
  ...
  {
    icon: <Percent className="w-8 h-8 sm:w-10 sm:h-10 text-green-600" />,
    title: "홀짝 & 패턴 분석",
    desc: "홀짝 비율과 패턴 흐름을 시각적으로 제공합니다.",
  },
  {
    icon: <Scan className="w-8 h-8 sm:w-10 sm:h-10 text-purple-600" />,
    title: "로또 용지 시각화",
    desc: "번호를 실제 로또 용지 형태로 직관적으로 확인할 수 있습니다.",
  },
  {
    icon: <Cpu className="w-8 h-8 sm:w-10 sm:h-10 text-red-600" />,
    title: "AI 번호 분석",
    ...
    >
      {features.map((f, i) => (
        <motion.div
          ...
          className="bg-white rounded-xl px-5 py-6 shadow-sm 
            flex flex-col items-center text-center
            border border-gray-100
            hover:border-blue-500/40 hover:shadow-blue-100
